...
export class SecurityTab extends React.PureComponent<Props, State> {
        ...
        if (active) {
            ...
            if (
                ...
                apps = this.state.authorizedApps.map((app) => {
                    ...
                    return (
                        <div key={app.id} className="pb-3 authorized-app">
    ...
    render() {
        ...
        numMethods = this.props.enableSignUpWithOffice365
            ? numMethods + 1
            : numMethods;
        numMethods = this.props.enableSignUpWithOpenId
            ? numMethods + 1
            : numMethods;
                ...
                <div className="user-settings">
                    ...
                    >
                        <LocalizedIcon
                            ...
                            title={{
                                id: t(
                                    "user.settings.security.viewHistory.icon"
                                ),
                    ...
                    >
                        <LocalizedIcon
                            ...
                            title={{
                                id: t(
                                    "user.settings.security.logoutActiveSessions.icon"
                                ),
...
export default function UserPreferencesSecurityTab(props: Props): JSX.Element {
    ...
    const [passwordSettings, setPasswordSettings] = useState<PasswordSettings>({
        ...
        confirmNewPassword: initialState(""),
    });
    const [showPassInputs, setShowPassInputs] = useState(false);

    const setupMfa = (e: React.MouseEvent<HTMLElement>) => {
        e.preventDefault();
    ...
    return (
        <>
            ...
            {showPassInputs &&
                Object.keys(passwordSettings).map((setting) => (
                    <>
                        <FieldsetItemCreator
                            title={setting}
                            value={passwordSettings[setting]?.value}
                            onChange={(e) =>
                                handleChange({
                                    key: setting,
                            ...
                            errors={passwordSettings[setting]?.errors}
            ...
            <SectionCreator
                title={ActiveSectionsTitle}
                description={ActiveSectionsDesc}
            />
            {showActiveSections()}
        </>
    );
